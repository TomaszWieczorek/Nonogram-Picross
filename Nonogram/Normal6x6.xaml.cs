...
namespace Nonogram
{
    ...
    /// </summary>
    public sealed partial class Normal6x6 : Page
    {
        private int i, j; //licznik
        
        int[] wIlosc = new int[6]; //tablica 5 wymiarowa posiadajaca ilosc pol w kluczu w wierszu
        int[] kIlosc = new int[6]; //tablica 5 wymiarowa posiadajaca ilosc pol w kluczu w kolumnie
        private bool[,] btnKlikniety = new bool[6, 6]; //tablica reprezentujaca wszystkie przyciski (cos w stylu btn.isChecked())
        private bool[,] klucz = new bool[6, 6]; //tablica reprezentujaca klucz odpowiedzi bedacy rozwiazaniem
        ...
        private void losowanie()
        {
            ...
            if (textIlosc.Text != "") //sprawdzamy czy uzytkownik wpisal liczbe, jesli nie jest losowana z przedzialu
            {
                ...
                if (iloscGracza < 10 || iloscGracza > 30) //sprawdzamy czy uzytkownik podal liczbe z przedzialu
                    ilosc = rnd.Next(15, 29); //jesli nie to ilosc pol w kluczu z przedzialu od 15(40%) do 28(80%)
                else
                    ilosc = iloscGracza; //jesli tak to ilosc gracza
            }
            else
            {
                ilosc = rnd.Next(15, 29); //ilosc pol w kluczu z przedzialu od 15(40%) do 28(80%)
            }
            i = 0;
            do
            {
                int randomI = rnd.Next(0, 6);
                int randomJ = rnd.Next(0, 6);
        ...
        private void kod_liter()
        {
            ...
            string[] opis = new string[12]; //tablica reprezentujaca 12 opisow, 0-5 wiersze, 6-11 kolumny
        ...
        {
            bool[] ulatwienieCzyOk = new bool[12]; //tablica reprezentujaca kwadraty przy ulatwieniach, 0-5 wiersze, 6-11 kolumny
            int kk, ww; //liczniki
            ...
            for (ww = 0; ww < 6; ww++)
            {
                ...
                if (czyOkK) ulatwienieCzyOk[ww + 6] = true;
                else ulatwienieCzyOk[ww + 6] = false;
            }
        ...
        private async void btnIlosc_Click(object sender, RoutedEventArgs e)
        {
            ...
            msgbox = new MessageDialog("Podaj liczbę z przedziału od 10 do 30!\nJeśli podasz z poza przedziału lub w ogóle nie podasz zostanie automatycznie wylosowana z przedziału (15~30)!");
